%% Begin
pl = plot_class();
sec_str = "Segundos";

%% Plot 1
out = sim("inversor_1.slx");
V_dc_follow = out.V_dcs;
V_dc_follow.signals.values = V_dc_follow.signals.values(:,[2,4]);
datas = {
    out.V_dcs, "Tensiónes de los condensadores", ["Voltios", sec_str], [], [];
    out.V_s, "Tensión AB", ["Voltios", sec_str], [], [];
    out.V_acs, "Tensiónes parciales", ["Voltios", sec_str], [], [];
    out.I_ab, "Intensidad AB", ["Voltios", sec_str], [], [];
};
pl.deal_datas(datas, [2, 2], "inversor_1_1");
frec = [7 - 0.09, 4, 50];
datas = {
    V_dc_follow, "Tensión del condensador 2", ["Voltios", sec_str], frec, [];
    out.V_s, "Tensión AB", ["Voltios", sec_str], frec, [];
    out.V_acs, "Tensiónes parciales", ["Voltios", sec_str], frec, [];
    out.I_ab, "Intensidad AB", ["Voltios", sec_str], frec, [];
};
pl.deal_datas(datas, [2, 2], "inversor_1_2");
frec = [2, 2000];
datas = {
    out.V_s, "Tensión AB", ["Voltios", sec_str], frec, [];
    out.V_acs, "Tensiónes parciales", ["Voltios", sec_str], frec, [];
};
pl.deal_datas(datas, [2, 1], "inversor_1_3");

%% Plot 2
out = sim("inversor_2.slx");
frec = [4, 50];
V_dc_follow = out.V_dcs;
V_dc_follow.signals.values = V_dc_follow.signals.values(:,[2,5]);
datas = {
    out.V_dcs, "Tensiónes de los condensadores", ["Voltios", sec_str], [], [];
    out.V_s, "Tensión AB", ["Voltios", sec_str], [], [];
    out.V_acs, "Tensiónes parciales", ["Voltios", sec_str], [], [];
    out.I_ab, "Intensidad AB", ["Voltios", sec_str], [], [];
};
pl.deal_datas(datas, [2, 2], "inversor_2_1");
datas = {
    V_dc_follow, "Tensión del condensador 2", ["Voltios", sec_str], frec, [];
    out.V_s, "Tensión AB", ["Voltios", sec_str], frec, [];
    out.V_acs, "Tensiónes parciales", ["Voltios", sec_str], frec, [];
    out.I_ab, "Intensidad AB", ["Voltios", sec_str], frec, [];
};
pl.deal_datas(datas, [2, 2], "inversor_2_2");
frec = [2, 2000];
datas = {
    out.V_s, "Tensión AB", ["Voltios", sec_str], frec, [];
    out.V_acs, "Tensiónes parciales", ["Voltios", sec_str], frec, [];
};
pl.deal_datas(datas, [2, 1], "inversor_2_3");

%% End
pause
close all
% clear
